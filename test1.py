...
class Trainer():
    ...
    def update(self, board, td_target, td_error, action, avaliableColumns, sess=None):
        ...
        _, loss, pol_loss, value_loss, mean_squared_target = sess.run([self.train_op, self.loss, self.policyLoss, self.valueLoss, self.policy.mean_squared_target], feed_dict)
...
class PolicyEstimator():
    ...
    def __init__(self, scope="policy_estimator", entropyFactor=0.1, shared_layers=None):
        with tf.variable_scope(scope):
            ...
            self.l1_relu = tf.nn.leaky_relu(features=self.l1, alpha=0.1)
            ...
            #
            self.mean_squared_target = tf.reduce_mean(tf.square(self.target))

            # Loss with TD-regularizing factor
            self.loss = -self.dist.log_prob(self.action) * (self.target - tf.math.rsqrt(self.mean_squared_target) * tf.square(self.target))
...
class ValueEstimator():
    ...
    def __init__(self, scope="value_estimator", dropout_keep_prob=1.0, shared_layers=None):
        with tf.variable_scope(scope):
            self.shared_layers = shared_layers
            if shared_layers is not None:
                # self.board = shared_layers.board
            ...
            self.l1_dropout = tf.nn.dropout(self.l1_relu, keep_prob=dropout_keep_prob)
            ...
            self.l2_dropout = tf.nn.dropout(self.l2_relu, keep_prob=dropout_keep_prob)
        ...
        for t in itertools.count():
            ...
            if t > 0:
                ...
                if batch_pos_X == batch_size:
                    ...
                    loss_X, policyLoss, valueLoss, meanSquaredTDError = trainer_X.update(batch_board_X, batch_td_target_X, batch_td_error_X, batch_action_X, batch_avaliableColumns_X)
                    loss_X = loss_X
                    ...
                    print("\nUpdates X network. Value loss: {}. Policy loss: {}. Mean squared TD Error: {}".format(valueLoss, policyLoss, meanSquaredTDError), end=" ")
                    stats.episode_value_loss[i_episode] += valueLoss

                    printClockTimeSince("Time since start of training:", startTime)
...
experienceReplayFile = 'experienceReplay5.pickle'
modelFile = "tmp/model11.ckpt"

batch_size = 100
numberOfGames = 10000

experienceReplayBatchSize = 500
experienceReplayNumberOfUpdates = 10000

global_step = tf.Variable(0, name="global_step", trainable=False)
conv_net = ConvolutionalNetwork("X_convNet")
policy_estimator = PolicyEstimator("X_policy", entropyFactor=0e-5, shared_layers=conv_net)
value_estimator = ValueEstimator("X_value", dropout_keep_prob=0.6, shared_layers=conv_net)
trainer = Trainer("X_trainer", learning_rate=1e-3, convNet=conv_net, policy=policy_estimator, policyLossFactor=1e-0, value=value_estimator, valueLossFactor=1e-0)
...
with tf.Session() as sess:
    ...
    stats = actor_critic(env, policy_estimator, value_estimator, trainer, numberOfGames, discount_factor=0.99, player2=True, positiveRewardFactor=1, negativeRewardFactor=1, batch_size=batch_size)

    experienceReplayStats = runExperienceReplay(experienceReplayFile, trainer, experienceReplayNumberOfUpdates, experienceReplayBatchSize)
